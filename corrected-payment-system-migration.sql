-- Corrected Payment System Migration for MyPartsRunner
-- This script handles the actual column names in your database
...
-- Step 2: Update existing orders with default values (using correct column names)
-- First, let's check what columns actually exist
DO $$
DECLARE
    col_exists BOOLEAN;
BEGIN
    -- Check if deliveryFee column exists (camelCase)
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'orders' AND column_name = 'deliveryFee'
    ) INTO col_exists;
    ...
    IF col_exists THEN
        ...
        RAISE NOTICE 'Updated using camelCase column names (deliveryFee)';
    ELSE
        ...
        RAISE NOTICE 'Updated using lowercase column names (deliveryfee)';
    END IF;
END $$;
