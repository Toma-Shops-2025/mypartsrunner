-- 🗄️ MYPARTSRUNNER COMPLETE DATABASE SETUP
...
CREATE TABLE IF NOT EXISTS driver_profiles (
    ...
    status TEXT CHECK (status IN ('offline', 'online', 'busy', 'break')) DEFAULT 'offline',
    createdAt TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updatedAt TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE POLICY "Anyone can view available drivers" ON driver_profiles FOR SELECT USING (isAvailable = true OR status = 'online');
